...
const myNavUI = {
    ...
    scrollTab: function (elem) {
        ...
        /**
         * createClickHandler, createScrollHandler
         * 고차 함수(클로저)로 클릭이벤트시 `container`, `scrollArea` 저장
         */
        tabNav.removeEventListener('click', myNavUI.handleClick);
        tabNav.addEventListener('click', myNavUI.createClickHandler(scrollArea));
        
        scrollArea.removeEventListener('scroll', myNavUI.handleScroll);
        scrollArea.addEventListener('scroll', myUI.debounce(myNavUI.createScrollHandler(container, scrollArea), 100));
    },
    createClickHandler: function (scrollArea) {
        return function (e) {
            const tabHandler = myUI.getTarget(e.target, 'tab-handler');

            if ( tabHandler ) {
                myNavUI.scrollToContent(tabHandler, scrollArea);
            }
        };
    },
    createScrollHandler: function (container, scrollArea) {
        return function () {
            myNavUI.focusingMenu(container, scrollArea);
        };
    },
    handleClick: function (event) {
        ...
        if ( tabHandler ) {
            myNavUI.scrollToContent(tabHandler, container);
        }
    },
    ...
    scrollTableInit: function (elem) {
        ...
        const tableWidth = tableContainer.querySelector('table').getBoundingClientRect().width;
        const prevElemHandlers = tableContainer.previousElementSibling;
        const nextElemHandlers = tableContainer.nextElementSibling;
        let scrollHandlers;
        if ( prevElemHandlers && prevElemHandlers.classList.contains('table-scroll-btns') ) {
            scrollHandlers = prevElemHandlers;
        } else if ( nextElemHandlers && nextElemHandlers.classList.contains('table-scroll-btns') ) {
            scrollHandlers = nextElemHandlers;
        }
        /** btnHandler
         * - 일반조건문:
         * let btnHandler = null;
         * if (scrollHandlers) { btnHandler = scrollHandlers.querySelector('button') || scrollHandlers.querySelector('a'); }
         * - 축약:
         * const btnHandler = scrollHandlers?.querySelectorAll('button') || scrollHandlers?.querySelectorAll('a') || null;
         * 항상 NodeList 객체를 반환하기 때문에 || 연산자가 의미가 없다. querySelectorAll()은 일치하는 요소가 없어도
         * 빈 NodeList를 반환하기 때문에, 둘 중 아무 것도 없을 때도 btnHandler는 무조건 truthy한 상태가 됨.
        ...
        if ( scrollHandlers ) {
            ...
            btnHandler = [...buttons, ...anchors]; // NodeList → 배열로 병합
        }
        const leftHandler = scrollHandlers.querySelector('.table-left-handler');
        const rightHandler = scrollHandlers.querySelector('.table-right-handler');
        const scrollInfo = { tableContainer, containerWidth, tableWidth, leftHandler, rightHandler }
        ...
        // 빈 상태를 고려하여 btnHandler && 추가
        btnHandler && btnHandler.forEach((btn) => {
            btn.addEventListener('click', function () {
                myNavUI.scrollTableBtn(btn, scrollInfo)
            })
        })
    },
    scrollTable: function (elem, scrollInfo) {
        const { tableContainer, containerWidth, tableWidth, leftHandler, rightHandler } = scrollInfo;

        if ( tableContainer.scrollLeft === 0 ) {
            leftHandler.classList.add('opacity-0')
            rightHandler.classList.remove('opacity-0')
        } else if ( Math.ceil(tableContainer.scrollLeft + containerWidth) >= tableWidth ) {
    ...
    scrollTableBtn: function (btn, scrollInfo) {
        const { tableContainer, tableWidth } = scrollInfo;

        if ( btn.classList.contains('table-right-handler') ) {
            tableContainer.scrollLeft = tableWidth;
        } else {
